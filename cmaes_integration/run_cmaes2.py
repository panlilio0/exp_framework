...
Authors: Thomas Breimer, James Gaskell, Guy Tallent
February 4th, 2025
"""
...
def run_cma_es(mode, gens, sigma_val):
    ...
    best_fitness_so_far = run_sim2.FITNESS_OFFSET
    ...
    for generation in range(gens):
        ...
        for indv_num in range(optimizer.population_size):
            x = optimizer.ask()
            fitness = run_sim2.run(NUM_ITERS, x, "h")
        ...
        if mode in ["s", "b", "v"]:
            ...
            run_sim2.run(NUM_ITERS, best_genome, mode, vid_name, vid_path)
