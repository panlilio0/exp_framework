...
class SNNController:
    ...
    def pad_outputs(self, tup, m):
        n = len(tup)
        if n >= m:
            return tup  # No padding needed if already >= m
        return tup + (0,) * (m - n)

    def _get_output_state(self, inputs, prev_values):
        """
        ...
        for snn_id, snn in enumerate(self.snns):
            input_values = inputs[snn_id] + tuple(list(itertools.chain.from_iterable(row[snn_id] for row in prev_values)))
            input_values = self.pad_outputs(input_values, self.inp_size)
            snn.compute(input_values)
    ...
    def get_lengths(self, inputs, prev_values):
        """
        Returns a list of target lengths (action array)
        """
        out = self._get_output_state(inputs, prev_values)
        lengths = []
